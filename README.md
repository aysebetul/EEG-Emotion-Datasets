...
5. [EEG Dataset for the Recognition of Different Emotions Induced in Voice-User Interaction](https://springernature.figshare.com/articles/dataset/Preprocessed_Dataset/25751097?backTo=/collections/EEG_Dataset_for_the_Recognition_of_Different_Emotions_Naturally_Induced_in_Voice-User_Interaction/7207839)
   ...
   *Device:* ActiChamp EEG Aplifier (Brain Products) (10-10 system)
   *Recordings:* EEG, ECG, PPG, GSR and facial expression. They applied notch-filter(59-61Hz) and bandpass filter(1-80Hz). Subsequently, EEG signal is downsampled form 1000Hz to 200 Hz. The preprocessed data is segmented for the "answer period" since they investigate the emotion change in response to the answer of VUI and emotion would be mostly induced in this period. 
   *Stimuli:* They aimed to create voice stimuli using Voice User Interface(VUI) system that mimics natural human to human communication. Two parameters are controlled in VUI system, voice type(child/adult) and information quantity(simple/detailed). User's satisfaction with the answers provided by VUI is considered as ground truth for induced emotion. They prepared 80 questions and answers. During experiment, participant ask the question to VUI by reading question and answer is given from among four types (child/simple, child/detailed, adult/simple, adult/detailed). 
   *Labels:* Participant defines the emotion based on some ciriteria such as interesting, obvious, impressive, etc. (each score -3 to 3)
   
6. [EAV](https://github.com/nubcico/EAV)
   [Paper](https://www.nature.com/articles/s41597-024-03838-4)
   *Data:* 42 participant, 30 channels, 200 interactions
   *Device:* 30 channels BrainAmp System (Brain Products) 
   *Recordings:* EEG, audio, video. EEG data recorded at a rate of 500 Hz. Additionally, they applied high-pass filter (0.5 Hz) and bandpass filter(50 Hz). Data is segmented into 20 seconds epochs. 
   *Stimuli:* In the experiments, participants are engaged in emotional dialogues with a conversational system. Therefore, dialogues have multiple iterations of "speaking" and "listening". Dialogues are generated by ChatGPT and edited by human to ensure the emotional clarity. Participant watch pre-recorded video (listening interaction) then respond to the corresponding dialogue by reading from monitor (speaking interaction). Each participant is included in 100 repeated interactions. Visual stimuli and dialogues are displayed during experiment. 
   *Labels:* Five distinct emotions - neutral, anger, happiness, sadness and calmness. Participant selected five dialogues (each has 4 interactions) for each emotional class so that class labels are established before the experiment. They also did self assessment to indicate their perceived emotion with arousal and valence score (-5 to 5) after each iteration. 
...
7. [MPED](https://github.com/Tengfei000/MPED/tree/datatset)
   ...
   *Data:* 23 participant, 62 channel, 14 trials, 28 videos
   *Device:* 62 channel ESI NeuroScan System (10-20 system)
   *Recordings:* EEG, ECG, Galvanic skin response, respiration. EEG sampling rate is 1000 Hz.
   *Stimuli:* Video clips. Video clips are selected after detailed experiments and assesed by three psychological questionnaires (PANAS, SAM, DES). 
   *Labels:* 7 discrete emotions. (neutral, joy, funny, disgust, anger, fear, sad). Additionally, after experiment, partiicipants conducted self assessment.
...
8. [DREAMER](https://zenodo.org/records/546113)
   ...
   *Recordings:* EEG and ECG. EEG sampling rate is 128 Hz. The Artefact Subspace Reconstruction method used to remove artefacts. 
   *Stimuli:* Audio and visual stimuli of film clips targeted 9 emotions (amusement, excitement, happiness, calmness, anger, disgust, fear, sadness and suprise)
...
9. [AMIGOSDataset](https://www.eecs.qmul.ac.uk/mmv/datasets/amigos/index.html)
   ...
   *Data:* Short video experiment: 40 participants, 14 channels, 16 videos, 16 trials
   Long videp experiment (individual): 17 participants, 14 channels, 4 videos, 2 trials
   Long videp experiment (group): 20 participants, 14 channels, 4 videos, 2 trials
   *Device:* Emotiv EPOC Neuroheadset
   *Recordings:* EEG, ECG, GSR, frontal HD video of participants. EEG data is recorded with rate of 128 Hz, signals are average-referenced and high-pass filter (2Hz) applied. 
   *Stimuli:* 16 short viodes and 4 long videos. 2 experiments: In the first one 40 participant watched 16 short emotional videos and in the second experiment, participants watched 4 long videos (some of them alone and rest in groups) Stimulus are annotated on valence and arousal dimension. 
...
10. [PEGCONV](https://pegconv.nastaran-saffar.me)
    ...
    *Recordings:* EEG, audio, video, GSR, PPG. EEG data recorded with the rate of 125 Hz. 
    *Stimuli:* Questions asked by interviewer (pre-intern psychologist) to induce the targeted emotion. 
    *Labels:* Five emotions: happiness, sadness, anger, fear, neutral state. Participants listened to the conversations and labeled their emotions using self-report questionnaires (scores 1-5). Each trial divided into three parts (beginning, in the middle, at the end), and collected three emotions for each trial.
...
11. [INTERFACES datasets](https://github.com/IoBT-VISTEC/EEG-Emotion-Recognition-INTERFACES-datasets)
    ...
    *Data:* 43 participant, 15 videos, 8 channels
    *Device:* OpenBCI (10-20 system)
    *Recordings:* EEG, EDA, BVP, Temp. EEG data is recorded with the rate of 250Hz. 
